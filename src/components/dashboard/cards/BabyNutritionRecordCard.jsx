...
import { getNutritionStatus, calculateBabyAgeInMonths } from "../../../utils/nutritionAlgorithm";
import EditModal from "../modals/EditModal";
import DeleteModal from "../modals/DeleteModal";

export default function BabyNutritionRecordCard(props) {
  const { record_number, id, growth_date, weight, height, head_circumference } =
    props;
  ...
  return (
    <>
      <tr className="border-b border-[#D1D9E2]">
        <td className="text-[#898989] text-center py-3.5">{record_number + 1}</td>
        <td className="text-[#898989] text-center py-3.5">
          {convertHumanReadDateFormat(growth_date)}
        </td>
        <td className="text-[#898989] text-center py-3.5">{weight}</td>
        <td className="text-[#898989] text-center py-3.5">{height}</td>
        <td className="text-[#898989] text-center py-3.5">{head_circumference}</td>
        <td className="text-[#898989] text-center py-3.5">
          <span className={`${getBgColorBasedOnNutrition(getConclusion())} font-bold text-sm text-white px-5 py-[6px] rounded-lg`}>
            {getConclusion()}
          </span>
        </td>
        <td className="text-[#898989] text-center py-3.5">
